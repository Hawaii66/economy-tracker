import { aggregateLedgerByTag } from 'budget-core'
...
export default function TagDetailView({ budgetId, tagId, state }: TagDetailViewProps) {
  ...
  const lifestyleTag = lifestyleTags.find((candidate) => candidate.id === tagId)
  const eventTag = eventTags.find((candidate) => candidate.id === tagId)
  const tag = lifestyleTag ?? eventTag
  const tagKind = lifestyleTag ? 'permanent' : eventTag ? 'temporary' : null
  ...
  const allTags = [
    ...lifestyleTags.map((entry) => ({ ...entry, kind: 'permanent' as const, archived: false })),
    ...eventTags.map((entry) => ({ ...entry, kind: 'temporary' as const })),
  ]

  if (!tag || !tagKind) {
    return (
      <div className="budget-page">
        <p className="m-0 text-sm text-[var(--text-muted)]">This tag could not be found.</p>
        <Link
  ...
  const spendingRows = toNamedAmounts(
    ...
    (id) => allTags.find((candidate) => candidate.id === id)?.name ?? 'Unknown tag',
    (id) => allTags.find((candidate) => candidate.id === id)?.color,
  ).slice(0, 8)
    ...
    <div className="budget-page">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="mb-1 flex flex-wrap items-center gap-2">
              <p className="kicker m-0">{tagKind === 'permanent' ? 'Permanent tag' : 'Event tag'}</p>
              {tagKind === 'temporary' && eventTag?.archived ? (
                <span className="rounded-full border border-[rgba(140,132,126,0.45)] px-2 py-0.5 text-xs font-semibold text-[var(--text-muted)]">
                  Archived
                </span>
              ) : null}
            </div>
            <h1 className="display-title m-0 text-2xl text-[var(--text)] sm:text-3xl">{tag.name}</h1>
          </div>
        </div>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <section className="budget-panel">
          ...
          {tagKind === 'temporary' && eventTag?.archived ? (
            <p className="mb-0 mt-4 text-sm text-[var(--text-muted)]">
              Archived tags stay out of suggestion lists but keep their historical ledger data.
            </p>
          ) : null}
        </section>
        ...
        <SpendingBarChart
          ...
          description="Top tagged expenses in this budget."
          rows={spendingRows}
          emptyMessage="Import and categorize transactions to see tag spending."
        />
      </div>
      ...
      <div className="mt-4">
        <FilteredLedgerPanel
          ...
          filters={filters}
          context={context}
          description="Ledger entries that include this tag."
          emptyMessage="No transactions with this tag yet."
        />
      </div>
    </div>
  )
